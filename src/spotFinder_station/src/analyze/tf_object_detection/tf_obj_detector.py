...
class TFObjDetector:
	...
	NUM_CLASSES = 90
	CWD_PATH = os.getcwd()
	MODEL_NAMES = ['ssd_inception_v2_coco_11_06_2017', 'rfcn_resnet101_coco_11_06_2017', 'faster_rcnn_resnet101_coco_11_06_2017', 'faster_rcnn_inception_resnet_v2_atrous_coco_11_06_2017']
	...
	def detect_obj(self, img_fname, viz= False):
		...
		(boxes, scores, classes, num_detections) = self.sess.run( [self.boxes, self.scores, self.classes, self.num_detections], feed_dict={self.image_tensor: image_np_expanded})
		
		lst_obj = [x for x in self.get_classes(np.squeeze(boxes), np.squeeze(classes).astype(np.int32), np.squeeze(scores), self.category_index)]
